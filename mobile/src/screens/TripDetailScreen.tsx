...
export const TripDetailScreen = () => {
  ...
  const handleContactDriver = () => {
    const companyPhone = typeof trip.company === 'object' ? trip.company.phone : '';

    if (!companyPhone) {
      Alert.alert(
        "Contacto no disponible",
        "Esta empresa no ha registrado su número de contacto. Por favor, intenta más tarde."
      );
      return;
    }

    const message = `Hola, estoy interesado en el viaje de ${routeName} el día ${trip.date} a las ${formatTimeAmPm(trip.departureTime)}.`;
    const url = `whatsapp://send?phone=57${companyPhone}&text=${encodeURIComponent(message)}`;

    Linking.canOpenURL(url).then(supported => {
      if (supported) {
        Linking.openURL(url);
      } else {
        Linking.openURL(`https://wa.me/57${companyPhone}?text=${encodeURIComponent(message)}`);
      }
    });
  };

  const isTerrestrial = ["carro", "moto", "bus"].includes(trip.transportType);
    ...
    <ScreenContainer withPadding={false}>
      <StyledView className="flex-1">
          ...
          <Card className="p-0 overflow-hidden">
            ...
            <StyledView className="p-6">
              ...
              <StyledView>
                ...
                {typeof trip.company === 'object' && trip.company.phone && (
                  <StyledText className="text-sm text-gray-500 mt-1">
                    Tel: {trip.company.phone}
                  </StyledText>
                )}
              </StyledView>
            </StyledView>
        ...
        <StyledView className="border-t border-gray-200 bg-white p-4">
          <Button
            title={isTerrestrial ? "Contactar Conductor" : "Seleccionar Asientos"}
            disabled={
              !trip.isActive ||
              (!isTerrestrial && soldSeats >= trip.capacity)
            }
            onPress={() => {
              if (isTerrestrial) {
                handleContactDriver();
