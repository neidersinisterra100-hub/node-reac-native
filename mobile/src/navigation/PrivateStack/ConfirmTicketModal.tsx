...
export default function ConfirmTicketModal() {
  ...
  const handleConfirm = async () => {
    try {
      ...
      /* =========================
         1. INICIAR COMPRA (BACKEND)
         ========================= */

      /**
       * buyTicket:
       * - crea ticket en pending_payment
       * - retorna datos para Wompi
       */
      const response = await buyTicket({
        tripId,
        passengerName: user?.name || "Pasajero",
        passengerId: "000000", // ⚠️ TODO: pedir documento al usuario
      ...
      /* =========================
         2. CONSTRUIR CHECKOUT URL
         ========================= */

      /**
       * En mobile:
       * - Usamos redirect externo
       * - Más estable que WebView
       */
      const checkoutUrl =
      ...
      if (!supported) {
        Alert.alert(
          "Error",
          "No se puede abrir la pasarela de pagos."
        );
        return;
      }
       ...
       * - El ticket aparecerá cuando Wompi notifique
       */
      Alert.alert(
        "Pago iniciado",
        "Se abrió la pasarela de pagos. Cuando completes el pago, tu ticket aparecerá automáticamente en la sección 'Mis Tickets'.",
        [
          {
            text: "Entendido",
            onPress: () => {
      ...
      <View style={styles.content}>
        ...
        <View style={styles.detailsCard}>
          ...
          <View style={styles.wompiContainer}>
            <Text style={styles.wompiText}>
              Pagos procesados de forma
              segura por
            </Text>
            <Text style={styles.wompiBrand}>
              WOMPI
            </Text>
          </View>
        </View>
